...
export const useReportStore = defineStore("report", {
  ...
  actions: {
    ...
    /**
     * Carga los valores de un reporte desde IndexedDB
     * y los vuelca en reportData o reportDataAnt,
     * agrupados por tablaorigen.
     *
     * @param {Object} payload
     * @param {number|string} payload.reporteid
     * @param {number|string} [payload.periodo]
     * @param {string} [payload.userid]
     * @param {boolean} [payload.isAnt=false]
     */
    async loadReportFromIndexedDB({ reporteid, periodo, userid, isAnt = false }) {
      const { getAllByIndex } = useIndexedDB()
      const targetState = isAnt ? this.reportDataAnt : this.reportData

      // Helper: filtra por periodo y usuario si te interesa
      const filterRows = rows =>
        (rows || []).filter(row => {
          if (periodo != null && row.periodo !== periodo) return false

          return !(userid && row.userid !== userid)
        })

      // Helper: agrupa por tablaorigen y arma objetos { [nombrecampo]: valor }
      const groupRowsByTabla = rows => {
        const result = {}

        for (const row of rows) {
          const tabla = row.tablaorigen
          if (!tabla) continue

          if (!result[tabla])
            result[tabla] = {}

          // nombrecampo será la key: ej. "efe_md_9501010107"
          const fieldKey = row.nombrecampo
          const numValue = Number(row.valor)

          result[tabla][fieldKey] = Number.isNaN(numValue) ? 0 : numValue
        }

        return result
      }

      // 1) Traer todos los valores por reporte desde cada store
      const [
        efemdRowsRaw,
        eriRowsRaw,
        esfRowsRaw,
        ...
        getAllByIndex("efemdvaluesconvertexs", "byReporte", reporteid),
        getAllByIndex("erivaluesconvertexs", "byReporte", reporteid),
        getAllByIndex("esfvaluesconvertexs", "byReporte", reporteid),
        getAllByIndex("ecpvaluesconvertexs", "byReporte", reporteid),
      ])

      // 2) Filtro por periodo / usuario (si los pasas)
      const efemdRows = filterRows(efemdRowsRaw)
      const eriRows   = filterRows(eriRowsRaw)
      const esfRows   = filterRows(esfRowsRaw)
      const ecpRows   = filterRows(ecpRowsRaw)

      // 3) Agrupo por tablaorigen
      const groupedEfemd = groupRowsByTabla(efemdRows)
      const groupedEri   = groupRowsByTabla(eriRows)
      const groupedEsf   = groupRowsByTabla(esfRows)
      const groupedEcp   = groupRowsByTabla(ecpRows)

      // 4) Mezclo todo en el estado (manteniendo compatibilidad)
      const mergeGrouped = grouped => {
        Object.entries(grouped).forEach(([tabla, campos]) => {
          targetState[tabla] = {
            ...(targetState[tabla] || {}),
            ...campos,
          }
      ...
      mergeGrouped(groupedEfemd)
      mergeGrouped(groupedEri)
      mergeGrouped(groupedEsf)
      mergeGrouped(groupedEcp)

      // Si quieres, también puedes setear "esfvalues", "erivalues" etc
      if (!isAnt) {
        this.esfvalues = groupedEsf
        this.erivalues = groupedEri
        this.ecpvalues = groupedEcp
        this.efemdvalues = groupedEfemd
      }
    },
  },
})
