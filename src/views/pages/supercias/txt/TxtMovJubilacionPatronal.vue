...
import { initializeMovJubilacionPatronal, initializeMovJubilacionPatronalAnt } from "@core/utils/initializers"

const reportStore = useReportStore()

const values = ref([])

const movJubilacionPatronal = initializeMovJubilacionPatronal()
const movJubilacionPatronalAnt = initializeMovJubilacionPatronalAnt()

/* PERIODO NUEVO */

const mov_saldoinicial = movJubilacionPatronal['mov_saldoinicial']
const mov_costosporservicios = movJubilacionPatronal['mov_costosporservicios']
const mov_costofinanciero = movJubilacionPatronal['mov_costofinanciero']
const mov_gananciaoperdidaactuarial = movJubilacionPatronal['mov_gananciaoperdidaactuarial']
const mov_otros = movJubilacionPatronal['mov_otros']
const mov_efectodeliquidaciones = movJubilacionPatronal['mov_efectodeliquidaciones']
const mov_beneficiospagados = movJubilacionPatronal['mov_beneficiospagados']
const mov_saldofinalsegunmovimiento = movJubilacionPatronal['mov_saldofinalsegunmovimiento']
const mov_diferenciaporcuadrar = movJubilacionPatronal['mov_diferenciaporcuadrar']

/* PERIODO ANTERIOR */
const mov_saldoinicial_ant = movJubilacionPatronalAnt['mov_saldoinicial_ant']
const mov_costosporservicios_ant = movJubilacionPatronalAnt['mov_costosporservicios_ant']
const mov_costofinanciero_ant = movJubilacionPatronalAnt['mov_costofinanciero_ant']
const mov_gananciaoperdidaactuarial_ant = movJubilacionPatronalAnt['mov_gananciaoperdidaactuarial_ant']
const mov_otros_ant = movJubilacionPatronalAnt['mov_otros_ant']
const mov_efectodeliquidaciones_ant = movJubilacionPatronalAnt['mov_efectodeliquidaciones_ant']
const mov_beneficiospagados_ant = movJubilacionPatronalAnt['mov_beneficiospagados_ant']
const mov_saldofinalsegunmovimiento_ant = movJubilacionPatronalAnt['mov_saldofinalsegunmovimiento_ant']
const mov_diferenciaporcuadrar_ant = movJubilacionPatronalAnt['mov_diferenciaporcuadrar_ant']

const convertirNegativo_efectodeliquidaciones_ant = async id => { mov_efectodeliquidaciones_ant.value = (Number(mov_efectodeliquidaciones_ant.value) != 0 && Number(mov_efectodeliquidaciones_ant.value) > 0) ? Number(mov_efectodeliquidaciones_ant.value) * -1 : Number(mov_efectodeliquidaciones_ant.value) }
const convertirNegativo_efectodeliquidaciones = async id => { mov_efectodeliquidaciones.value = (Number(mov_efectodeliquidaciones.value) != 0 && Number(mov_efectodeliquidaciones.value) > 0) ? Number(mov_efectodeliquidaciones.value) * -1 : Number(mov_efectodeliquidaciones.value) }
const convertirNegativo_beneficiospagados = async id => { mov_beneficiospagados.value = (Number(mov_beneficiospagados.value) != 0 && Number(mov_beneficiospagados.value) > 0) ? Number(mov_beneficiospagados.value) * -1 : Number(mov_beneficiospagados.value) }

const mov_saldofinalsegunesf_ant = computed(() => {
  return (Number(reportStore.getSingleReportValue("pasivoscorrientesifluc_ant", "esf_2011201_ant", true)) + Number(reportStore.getSingleReportValue("pasivosnocorrientesifluc_ant", "esf_2020701_ant", true))).toFixed(2)
})

const mov_saldofinalsegunesf = computed(() => {
  return (Number(reportStore.getSingleReportValue("pasivoscorrientesifluc", "esf_2011201")) + Number(reportStore.getSingleReportValue("pasivosnocorrientesifluc", "esf_2020701"))).toFixed(2)
})
...
const handleActionClick = async id => {
  ...
  reportStore.updateDiferenciaporcuadrar_jp(mov_diferenciaporcuadrar.value)
  reportStore.saveToLocalStorage("diferenciaporcuadrar_jp", mov_diferenciaporcuadrar.value)

  const movjubilacionpatronal = {
  ...
  reportStore.setReportData({
    ...
    movjubilacionpatronal: movjubilacionpatronal,
  })
}
...
const handleActionClick_ant = async id => {
  ...
  reportStore.updateDiferenciaporcuadrar_jpAnt(mov_diferenciaporcuadrar_ant.value)
  reportStore.saveToLocalStorage("diferenciaporcuadrar_jp_ant", mov_diferenciaporcuadrar_ant.value)

  const movjubilacionpatronal_ant = {
  ...
  reportStore.setReportDataAnt({
    ...
    movjubilacionpatronal_ant: movjubilacionpatronal_ant,
  })
}
          ...
          <VRow>
            <VCol cols="12" md="12" style="font-size:8px;">
              ...
              <VRow class="font-weight-medium px-4 mt-10">
                ...
                <VCol cols="12" md="3">
                  <VTextField label="(-) beneficios pagados" @blur="convertirNegativo_beneficiospagados(this); handleActionClick_ant(this)" @input="handleActionClick_ant(this)" @keyup="handleActionClick_ant(this)" type="number" v-model="mov_beneficiospagados_ant" />
                </VCol>
              </VRow>
